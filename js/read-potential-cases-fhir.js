...
(async () => {
  ...
  await fs.writeFile("multiple-sclerosis-potential-cases.csv", "patient-id,dob,codes,last-encounter\n");
  for(let patient in patients) {
    try {
      ...
      await fs.appendFile("multiple-sclerosis-potential-cases.csv", row);
    } catch(error) {
      console.log(error);
    }
  }
